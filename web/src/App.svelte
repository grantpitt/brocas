<script lang="ts">
  ...
  let interim = "";
  $: [displayTranscript, displayInterim] = formatTranscriptToDisplay(
    transcriptChunks,
    interim
  );
  ...
  async function cleanAndPredict(rawTranscript: string) {
    ...
    const responce = await fetch(`${import.meta.env.VITE_SERVER_URL}/clean`, {
      method: "POST",
      headers: {
  ...
  function formatTranscriptToDisplay(
    ...
    const numTranscriptWords = Math.max(
      0,
      numWords - displayIntermWords.length
    );
    const displayTranscriptWords =
      numTranscriptWords === 0
        ? []
        : transcriptWords.slice(-numTranscriptWords);
