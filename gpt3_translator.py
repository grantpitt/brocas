...
class PrepText:
    ...
    def parse_prompt(self, first, file_name=None):
        ...
        if file_name is None:
            file_name = (
                "./transcripts/prep_text/simple.txt"
                if first
                else "./transcripts/gpt3/test_user/simple_a.txt"
            )
        else:
...
class GPT3_Interfacer:
    ...
    def predict(self, prompt):
        prompt = (
            'Give the next words in the sentence:\n"I am going" <to town>\n"'
            + prompt
            + '" <'
        )
        response = openai.Completion.create(
